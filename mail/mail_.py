...
def confirmMail():
    name = 'Zoe' # request.args.get('name')
    receiver = 'thankyou0829@gmail.com' # request.args.get('email')
    ...
    artist = 'Twice' # json.loads(r.text).get('artist')
    date = '01/11' # json.loads(r.text).get('date')
    time = '19:00' # json.loads(r.text).get('time')
